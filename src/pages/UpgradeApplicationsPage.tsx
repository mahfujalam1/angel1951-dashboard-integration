...
const UpgradeApplicationsPage: React.FC = () => {
  ...
  const { data, isLoading, isError } = useGetUpgradeApplicationsQuery({ page, limit: 10 });
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any) => (
        <div>
          ...
          <div className="text-xs text-slate-400">{record.contactEmail || record.email}</div>
        </div>
      ),
    },
    {
      ...
      render: (_: any, record: any) => (
        <Space>
          <Button
            type="text"
            icon={<Eye size={16} />}
            onClick={() => navigate(`/upgrade-applications/${record.id}`)}
          />
  ...
  return (
    <div className="space-y-6">
      <Card title="Tier Upgrade Applications">
        {isError && (
          <div className="mb-4 text-sm text-red-500">Failed to load upgrade applications.</div>
        )}
        <Table
          dataSource={list}
          columns={columns}
