...
export default function NigeriaToAbroadForm() {
  ...
  const inputClass = "w-full border-none bg-slate-100/50 rounded-md px-3 py-2.5 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:bg-slate-100 transition-colors";
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 max-w-4xl mx-auto pb-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            <div className="w-full bg-slate-100/50 border-none rounded-md px-3 py-2.5 text-sm text-slate-600 flex items-center gap-2">
              ...
              <span className="ml-auto text-[10px] bg-slate-200 px-2 py-0.5 rounded text-slate-500 uppercase font-bold tracking-wider">Fixed</span>
            </div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-6">
              {(["Yes", "No"] as const).map((v) => (
                <label key={v} className="flex items-center gap-2 text-sm font-medium text-slate-600 cursor-pointer">
                  <input type="radio" value={v} {...register("insurance")} className="w-4 h-4 text-blue-600 border-slate-300" />
                  {v}
                </label>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input type="number" min="0" step="0.01" {...register("packageKg", { required: "Required" })} placeholder="Enter weight in KG" className={inputClass} />
              <div className="flex items-center px-3 bg-slate-100/50 border-none rounded-md text-slate-500 text-sm font-medium min-w-[60px] justify-center">KG</div>
            </div>
          </div>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
        <h2 className={sectionTitle}><span className="w-1.5 h-6 bg-blue-600 rounded-full" /> Pickup & Package Image</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="md:col-span-2">
          ...
          <div className="md:col-span-2">
            ...
            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-slate-200 rounded-2xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all duration-200">
              ...
              <span className="text-sm text-slate-500 font-medium tracking-tight">Click to upload parcel photo</span>
      ...
      <div className="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
        <h2 className={sectionTitle}><span className="w-1.5 h-6 bg-blue-600 rounded-full" /> Payment Information</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className={labelClass}>Service Type</label>
            <select {...register("paymentServiceType", { required: "Required" })} className={inputClass}>
              <option value="">Select</option>
          ...
          <div>
            ...
            <select {...register("paymentCurrencyType", { required: "Required" })} className={inputClass}>
              <option value="">Select</option>
          ...
          <div className="md:col-span-2">
            ...
            <input {...register("paymentAmountPayable", { required: "Required" })} placeholder="Enter amount" className={inputClass} />
          </div>
        </div>
      </div>

      <div className="flex justify-center pt-4">
        <button type="submit" className="w-64 py-4 bg-blue-600 hover:bg-blue-700 text-white text-base font-bold rounded-full transition-all shadow-lg shadow-blue-100">
          Send Invoice
        </button>
      </div>
    </form>
